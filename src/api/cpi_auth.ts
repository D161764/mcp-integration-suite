...
export const getOAuthTokenCPI = async (): Promise<DestinationAuthToken> => {
	...
	params.append("client_id", getEnvValue("CPI_OAUTH_CLIENT_ID") as string);
	params.append(
		"client_secret",
		getEnvValue("CPI_OAUTH_CLIENT_SECRET") as string
	);

	const response = await fetch(getEnvValue("CPI_OAUTH_TOKEN_URL") as string, {
		method: "POST",
		headers: {
